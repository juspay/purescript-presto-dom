...
import PrestoDOM.Types.Core (class Loggable, performLog, PrestoWidget(..), Prop, ScopedScreen, Controller, ScreenBase, Screen)
import PrestoDOM.Utils (continue, logActionImpl, addTime2, performanceMeasure)
...
import PrestoDOM.Core.Utils (callMicroAppsForListState, extractAndDecode, extractAndDecode', extractJsonAndDecode, forkoutListState, generateCommands, getListData, replayListFragmentCallbacks', verifyFont, verifyImage, attachUrlImages, isListContainer)
...
foreign import storeMachine :: forall a. EFn.EffectFn3 a String String Unit
...
domAll {name, parent} ids dom = {--dom--} do
  ...
  case hush $ runExcept $ decode d of
    Just (vdomTree :: VdomTree) -> do
      ...
      subScreenProps <- pure $ unsafeCoerce vdomTree.screen
      ...
      case subScreenProps of 
        Nothing ->
          pure $ generateCommands $ vdomTree {children = children, props = props}
        Just ssProps -> do
          let sProps = encodeInScreen ssProps
          case vdomTree.machine of
            Just machine' -> do
              liftEffect $ setUpBaseState (fromMaybe "" sProps.controller.parent) $ encode (Nothing :: Maybe String)
              void $ liftEffect $ EFn.runEffectFn3 storeMachine machine' sProps.controller.name (fromMaybe "" sProps.controller.parent)
              void $ forkAff $ runControllerImpl sProps.controller (unsafeCoerce {}) sProps.performLog sProps.show
              domAll {name : sProps.controller.name, parent : sProps.controller.parent} i (extract $ unsafeCoerce machine')
            Nothing ->
              pure $ encode (Nothing :: Maybe VdomTree)
    a -> pure $ encode a
...
controllerActions :: forall action state returnType a
  ...
  -> (Object Foreign)
  -> (state -> Effect Unit)
  -> (action -> Object Foreign -> Effect Unit)
  -> (action -> String)
  -> (Either Error returnType -> Effect Unit)
  -> Effect Canceler
controllerActions {event, push} {initialState, eval, name, globalEvents, parent} json emitter pl s cb = do
  ns <- sanitiseNamespace parent
  ...
  pure $ effectCanceler (EFn.runEffectFn2 cancelExistingActions name ns)
    where
      onStateChange previousAction currentAction timerRef (Tuple state cmds) = do
        ...
        logActionImpl timerRef previousAction currentAction false json pl s -- debounce
      ...
      onExit previousAction currentAction timerRef (Tuple sc ret) = do
        ...
        cb $ Right ret
        logActionImpl timerRef previousAction currentAction true json pl s-- logNow
      registerEvents =
        (\f -> f push)
      execEval action st =
...
runScreen st@{ name, parent, view} json = do
  runScreenImpl st json performLog show

runScreenImpl :: forall action state returnType
  . ScopedScreen action state returnType
  -> (Object Foreign)
  -> (action -> Object Foreign -> Effect Unit)
  -> (action -> String)
  -> Aff returnType
runScreenImpl st@{ name, parent, view} json pl s = do
  ns <- liftEffect $ sanitiseNamespace parent
  ...
  makeAff $ controllerActions eventIO st json (patchAndRun name parent (view eventIO.push)) pl s
...
runController st@{name, parent, eval, initialState, globalEvents, emitter} json =  
  runControllerImpl st json performLog show

runControllerImpl :: forall action state returnType
  . Controller action state returnType
  -> (Object Foreign)
  -> (action -> Object Foreign -> Effect Unit)
  -> (action -> String)
  -> Aff returnType
runControllerImpl st@{name, parent, eval, initialState, globalEvents, emitter} json pl s = do
  ns <- liftEffect $ sanitiseNamespace parent
  _ <- liftEffect $ setControllerStates ns name
  eventIO <- liftEffect $ getEventIO name parent
  makeAff $ controllerActions eventIO st json emitter pl s
...
showScreen st@{name, parent, view} json = do
  showScreenImpl st json performLog show

showScreenImpl :: forall action state returnType
  . ScopedScreen action state returnType
  -> (Object Foreign)
  -> (action -> Object Foreign -> Effect Unit)
  -> (action -> String)
  -> Aff returnType
showScreenImpl st@{name, parent, view} json pl s = do
  ns <- liftEffect $ sanitiseNamespace parent
  makeAff (\cb -> Efn.runEffectFn3 awaitPrerenderFinished ns name (cb $ Right unit) $> nonCanceler )
  liftEffect $ EFn.runEffectFn2 checkAndDeleteFromHideAndRemoveStacks ns name
...
encodeInScreen :: forall action state returnType. Foreign -> { performLog :: action -> (Object Foreign) -> Effect Unit
                                                             , controller :: Controller action state returnType 
                                                             , show :: action -> String 
                                                             }
encodeInScreen = unsafeCoerce